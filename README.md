...
需要 Python 3.8 以上。打开终端输入 `python --version` 检查，如果提示找不到去 [python.org](https://www.python.org/) 下载安装。
...
Flask 是唯一需要的 Python 库。如果只有这一台电脑用，直接装全局：
...
激活虚拟环境后终端前面会显示 `(.venv)`，说明现在用的是虚拟环境里的 Python。
...
> 如果手机打不开，可能是防火墙拦了。Windows 可以去 控制面板 -> Windows Defender 防火墙 -> 允许应用通过防火墙，把 Python 加进去。
